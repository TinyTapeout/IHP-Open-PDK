...
foreach dev $devices {
	...
	# Ignore these properties
	property "-circuit1 $dev" delete as ad ps pd mult sa sb sd nf ng nrd nrs
    }
    if {[lsearch $cells2 $dev] >= 0} {
	...
	# Ignore these properties
	property "-circuit2 $dev" delete as ad ps pd mult sa sb sd nf ng nrd nrs
    }
}
...
foreach dev $devices {
	...
	property "-circuit1 $dev" tolerance {w 0.01} {l 0.01}
	# Ignore these properties
	property "-circuit1 $dev" delete as ad ps pd mult sa sb sd nf ng nrd nrs area perim topography
    }
    if {[lsearch $cells2 $dev] >= 0} {
	property "-circuit2 $dev" parallel enable
	property "-circuit2 $dev" parallel {l critical}
	...
	property "-circuit2 $dev" tolerance {w 0.01} {l 0.01}
	# Ignore these properties
	property "-circuit2 $dev" delete as ad ps pd mult sa sb sd nf ng nrd nrs area perim topography
    }
}
